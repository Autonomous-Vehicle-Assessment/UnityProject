...
public class PathLoader : MonoBehaviour
{
    ...
    public void WriteTextFile()
    {
        if (pathWrite != null && pathWrite.Length > 0)
        {
            ...
            WaypointPath[] aiPath = transform.GetComponentsInChildren<WaypointPath>();

            List<string> pathList = new List<string>();
            pathList.Add("Name;R;G;B;X;Z;Vel");
            foreach (WaypointPath path in aiPath)
            {
                string pathString = "";
    ...
    public void GeneratePath()
    {
        ...
        path.AddComponent<WaypointPath>();
        path.GetComponent<WaypointPath>().lineColor = pathColor;
